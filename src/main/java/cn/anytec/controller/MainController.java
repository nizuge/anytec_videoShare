...
public class MainController{
    ...
    @ResponseBody
    public String updateBGM(@RequestParam("bgm")MultipartFile audio, @RequestParam("location")String place,HttpServletResponse response){
        ...
        String result = expZoneService.updateBgm(audio,place);
        if(result.equals("400")) {
            resultMap.put("code","can't find audio");
            response.setStatus(400);
            return new JSONObject(resultMap).toJSONString();
        }else if(result.equals("500")) {
            logger.info("500,更新BGM失败");
            response.setStatus(500);
            resultMap.put("code","更新BGM失败");
            return new JSONObject(resultMap).toJSONString();
        }else {
        logger.info("update bgm success");
        resultMap.put("code","success");
        return new JSONObject(resultMap).toJSONString();
        }
    }
